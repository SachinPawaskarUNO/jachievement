...
@section('content')
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <div class="pull-left">
                <a href="{{ URL::route('comments.index') }}" class="btn btn-info">Back</a>
            </div>
            ...
            <div class="pull-right">
                <form action="{{ url('comments/'.$comment->id) }}" method="POST"  onsubmit="return ConfirmDelete();">{{ csrf_field() }}{{ method_field('DELETE') }}
                    <button style="height: 24px;" type="submit" id="delete-comment-{{ $comment->id }}" class="btn btn-danger">Delete</button>
                </form>
            </div>
            <div class="pull-right">
                <a href="{{ URL::route('comments.edit', ['id' => $comment->id, 'method' => 'GET']) }}" class="btn btn-primary">Edit</a>
            </div>
            @endif
