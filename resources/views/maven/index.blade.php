    ...
    @if($maven_items->count() > 0)
        <table class="table">
            ...
            <tr>
                <th>#</th>
                <th>
                    <nobr>@lang('maven.question_and_answer')</nobr>
                </th>
                <th>Date of Creation</th>
                <th class="text-right">
                    <div class="dropdown">
                        <button class="btn btn-info btn-sm dropdown-toggle" type="button" id="dropdownMenu1"
                ...
                <tr>
                    ...
                    <td style="line-height: 1.8em;">
                        @if(!is_null($maven_item->faq))
                            <span class="label label-success">Q</span> <span
                                    style="text-decoration: underline;background:#eee;">{{ str_limit($maven_item->faq->raw_question, 100) }}</span>
                            <br>
                            ...
                            @if($maven_item->faq->tags->count() > 0)
                                <div>
                                    @foreach($maven_item->faq->tags as $maven_tag)
                                        <a href="?tag={{ urlencode($maven_tag->tag) }}"
                                           class="btn btn-xs btn-default">{{ $maven_tag->tag }}</a>
                                    @endforeach
                                </div>
                            @endif
                        @endif
                    ...
                    <td class="text-right">
                        ...
                        <button class="btn btn-danger btn-sm remove-button" data-id="{{ $maven_item->id }}">Delete
                        </button>
                    </td>
                </tr>
            @endforeach
